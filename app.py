...
###ADDITIONAL CODE FROM LAUREN
parks_ev = Base.classes.parks_ev
...
##############################################################################################################
#####CODE ADDED BY LAUREN
# Route that will trigger the scrape function
@app.route("/scrape")
def scrape():
    session = Session(engine)
    results = session.query(parks_ev).all()
    events = list(np.ravel(results))
    session.close()
    return render_template("index.html", parkev=events)
